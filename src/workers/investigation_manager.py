...
class InvestigationManager:
    ...
    def __init__(self, input_q: Queue, output_q: Queue, debug_q: Queue, config: dict):
        ...
        self.active_endpoints: Set[str] = set() # 从跟踪URL改为跟踪端点
        ...
        try:
            while True:
                ...
                if event_type == 'user_interaction' and execution_mode == 'passive':
                    await self.interaction_worker.handle_interaction_event(event)
    ...
    async def _handle_navigation_event(self, nav_info: dict, execution_mode: str):
        ...
        # 1. 对所有导航事件，都进行一次URL参数分析
        asyncio.create_task(self.interaction_worker.analyze_url(nav_info))

        # 2. 只有在新端点第一次出现时，才启动一个长期运行的AgentWorker来维持被动环境
