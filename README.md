...
- [ ] **增强“影子调查员”能力 (Enhance "Shadow Investigator" Capabilities)**
  - **描述**: 实现`docs/shadow_browser_design.md`中描述的高级功能，例如为用户的每一个操作，在后台自动生成并执行多种安全测试变体（如XSS、SQLi、IDOR等），实现真正的、用户无感的主动模糊测试。
  - **价值**: 将被动分析提升为主动、深度的安全测试，极大增强漏洞发现能力。

- [ ] **深化CDP调试器与IAST集成 (Deepen CDP & IAST Integration)**
  - **描述**: 完善`AgentWorker`与`InteractionWorker`，使其能主动消费和理解来自`debug_events_q`的实时调试和IAST事件。将运行时上下文作为核心情报，送入AI的决策`prompt`中。
  - **价值**: 让AI具备“读懂”代码执行逻辑的能力，能够发现复杂的、与业务逻辑相关的漏洞，实现`docs/cdp_network_ai_fusion.md`中的设想。

- [ ] **实现真正的并行测试 (Implement True Parallel Testing)**
  - **描述**: 修改`AgentWorker`的自主模式，使其能够将决策出的动作（如点击、输入）同时在多个并行的浏览器实例中执行，并对返回结果进行差异比对。
  - **价值**: 模仿`docs/autonomous_mode_diagrams.md`中的设计，通过差异分析来检测非预期的行为、权限绕过或数据泄露。

- [ ] **实现AI工作记忆总结 (Implement AI Working Memory Summarization)**
  - **描述**: 为解决大模型长上下文的限制，在`AgentWorker`的循环中增加一个“自我总结”步骤。AI会周期性地将其冗长的操作历史（`history`）压缩成一个简短的状态摘要。
  - **价值**: 确保AI在进行长期、多步骤的调查时，不会因为历史记录过长而丢失关键信息或超出上下文限制，提升长期任务的稳定性和效率。

### 侦察与攻击能力增强 (细化)

- [ ] **增强侦察与指纹识别能力 (Enhanced Reconnaissance & Fingerprinting)**
  - **描述**: 为AI增加多种信息收集的“感官”，为后续的DAST引擎提供精确制导信息。
  - **待办 (TODO)**:
    - [ ] **HTTP安全头分析**: 检查CSP, HSTS, CORS等安全头的配置，评估基础安全水平。
    - [ ] **客户端存储分析**: 扫描LocalStorage和SessionStorage，寻找泄露的敏感信息（Token, PII等）。
    - [ ] **第三方组件分析 (SCA)**: 识别引用的JS库及其版本，关联CVE数据库，寻找已知漏洞。
    - [ ] **网站元文件分析**: 自动抓取和解析`robots.txt`, `sitemap.xml`，发现隐藏的测试路径或管理后台。
    - [ ] **网站图标哈希指纹**: 通过计算Favicon哈希，识别常见的Web应用或框架（如Jenkins, Spring Boot等）。

- [ ] **构建高级DAST引擎 (PoC-Driven DAST Engine)**
  - **描述**: 将当前的DAST测试升级为一个可编排、支持复杂PoC的工作流引擎。该引擎将负责执行由“侦察与指纹识别”模块发现的攻击面，使其接近Nuclei等专业工具的能力。
  - **已完成**: 
...
- [ ] **优化导航事件处理与增量分析 (Optimize Navigation & Incremental Analysis)**
  - **描述**: 引入“URL端点规范化”逻辑，以区分“新页面导航”和“同一页面的参数状态变化”。
  - **价值**: 避免对同一页面的重复性重量级分析，极大提升被动扫描模式的效率和智能性，将资源集中在刀刃上。

- [ ] **增强启动逻辑与多标签页处理 (Robust Startup & Multi-Tab Handling)**
  - **描述**: 优化程序启动时的行为。当前，Aegis只监听启动后新发生的导航，并从第一个标签页复制认证。优化后的逻辑将：1. 在启动时扫描所有已打开的标签页。2. 智能地从已登录的、在白名单内的标签页中复制认证状态。
  - **价值**: 极大地改善用户体验，完美支持“先打开并登录多个目标网站，再启动Aegis”的工作流。
